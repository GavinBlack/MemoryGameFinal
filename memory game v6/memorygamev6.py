...
class Card(games.Sprite):
        ...
        def cardClicked(self):
                if not Card.gameover: #if the game is not over
                                ...
                                if Card.time == games.screen.fps * 8: #timer for letting the cards display
                                        for card in Card.clickedCards:
                                                ...
                                                if Card.clickedCards[0].whichImage == Card.clickedCards[1].whichImage:
                                                        card.destroy()
                                                        self.world.score.value = "Score: {}".format(Card.score+10)
                                                        ...
                                                        if not Card.alreadySetLives: #If already set lives once, don't do it again
                                                                Card.tries += 1
                                                                World.lifeCounter.value = "Lives: {}".format(int(Card.easyLives-Card.tries))
                                                                Card.alreadySetLives = True
                                ...
                                if Card.time == games.screen.fps * 8: #timer for letting the cards display
                                        for card in Card.clickedCards:
                                                ...
                                                if Card.clickedCards[0].whichImage == Card.clickedCards[1].whichImage and Card.clickedCards[0].whichImage == Card.clickedCards[2].whichImage:
                                                        card.destroy()
                                                        self.world.score.value = "Score: {}".format(Card.score+15)
                                                        self.world.totalCards -= 1
                                                        ...
                                                        if not Card.alreadySetLives: #If already set lives once, don't do it again
                                                                Card.tries += 1
                                                                World.lifeCounter.value = "Lives: {}".format(int(Card.hardLives-Card.tries))
                                                                Card.alreadySetLives = True
                                                        
                                                        if int(Card.totalAttempts/3) == Card.hardLives:
                                                                Card.gameover = True
                                                                gameover = games.Message(value = "Game Over",
                                                                                size = 70,
                                                                                color = color.red,
...
class World(object):
        ...
        lifeCounter = games.Text(value = "",
                                size = 30,
                                color = color.green,
                                top = 30,
                                right = games.screen.width - 60,
                                is_collideable = False)
        games.screen.add(lifeCounter)
        ...
        def createBoard(self, difficulty):
                ...
                if TkPygame.difficulty == 1:
                        World.lifeCounter.value = "Lives: {}".format(Card.easyLives)
                else:
                        World.lifeCounter.value = "Lives: {}".format(Card.hardLives)
                World.score.value = "Score: 0"
                
                if not World.alreadyDisplayed: #display how many cards you need to match in the beginning
                        if difficulty == 1:
...
class TkPygame:
    ...
    def make_tk_widgets(self):
        ...
        self.current_difficulty_label = ttk.Label(self.tk_root,text="Current Difficulty: Easy")
        self.current_difficulty_label.grid(row=5,column=3,padx=30,pady=15)
    ...
    def change_difficulty(self, difficulty):
        if difficulty == 1:
            ...
            self.current_difficulty_label['text'] = "Current Difficulty: Easy"
        else:
            TkPygame.difficulty = 2
            self.current_difficulty_label['text'] = "Current Difficulty: Hard"
